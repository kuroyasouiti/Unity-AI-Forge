    ...
    server = Server(
        ...
        instructions="\n".join(
            [
                ...
                "",
                "【ベクタースプライト生成（プロトタイプ用）】",
                "外部アセット不要でスプライトを即座に生成。ゲームジャム、プロトタイピング、プレースホルダーに最適。",
                "1. プリミティブ図形からスプライト生成",
                "   - unity.vectorSprite.convert(operation='primitiveToSprite', primitiveType='circle', color={'r': 1.0, 'g': 0.0, 'b': 0.0, 'a': 1.0}, outputPath='Assets/Sprites/RedCircle.png')",
                "   - 対応図形: circle（円）, square（正方形）, rectangle（長方形）, triangle（三角形）, polygon（多角形）",
                "   - 色指定: 辞書形式 {'r': 0-1, 'g': 0-1, 'b': 0-1, 'a': 0-1} または Hex形式 '#FF0000'",
                "   - サイズ指定: width, height（デフォルト256ピクセル）",
                "",
                "2. 単色スプライト生成（UIプレースホルダー）",
                "   - unity.vectorSprite.convert(operation='createColorSprite', width=64, height=64, color={'r': 0.5, 'g': 0.5, 'b': 0.5, 'a': 1.0}, outputPath='Assets/UI/Placeholder.png')",
                "   - ボタン背景、パネル、UIプレースホルダーに最適",
                "",
                "3. SVGからスプライト変換（Unity 2021.2+）",
                "   - unity.vectorSprite.convert(operation='svgToSprite', svgPath='Assets/Vectors/Icon.svg', outputPath='Assets/Sprites/Icon.png', width=512, height=512)",
                "   - プロジェクト内のSVGファイルをスプライトに変換",
                "",
                "4. テクスチャからスプライト変換",
                "   - unity.vectorSprite.convert(operation='textureToSprite', texturePath='Assets/Textures/Character.png', pixelsPerUnit=100, filterMode='point')",
                "   - 既存のテクスチャをスプライトとして設定",
                "   - filterMode: 'point'（ピクセルアート）、'bilinear'（標準）、'trilinear'（高品質）",
                "",
                "【作業の流れの推奨手順】",
