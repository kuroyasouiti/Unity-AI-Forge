...
Assets/
  Scripts/
    ...
    UI/
      HandList.cs              # 生成: unity_gamekit_ui (widgetType=list)
      DiscardList.cs           # 生成: unity_gamekit_ui (widgetType=list)
      PlayAreaSlot.cs          # 生成: unity_gamekit_ui (widgetType=slot)
      TargetSelection.cs       # 生成: unity_gamekit_ui (widgetType=selection)
      CommandPanel.cs          # 生成: unity_gamekit_ui (widgetType=command)
      ManaBinding.cs           # 生成: unity_gamekit_ui (widgetType=binding)
      HPBinding.cs             # 生成: unity_gamekit_ui (widgetType=binding)
      DeckCountBinding.cs      # 生成: unity_gamekit_ui (widgetType=binding)
  Data/
    Cards/
...
# 装備スロット（武器・防具）
unity_gamekit_ui(widgetType='slot',operation='create',
...
`unity_gamekit_ui(widgetType='binding')` で `deck_count` を UI に連動させる。
...
`unity_gamekit_ui(widgetType='list',operation='addItem')` で順次呼び出す。
アニメーション演出が必要な場合はカスタムスクリプトでドロー演出を付加する。
...
| UI | `unity_gamekit_ui(widgetType='list')` | 手札・墓地・コレクション |
| UI | `unity_gamekit_ui(widgetType='slot')` | フィールド・装備スロット |
| UI | `unity_gamekit_ui(widgetType='selection')` | ターゲット選択・フィルタタブ |
| UI | `unity_gamekit_ui(widgetType='command')` | ターン終了・降参 |
| UI | `unity_gamekit_ui(widgetType='binding')` | マナ・HP・デッキ枚数表示 |
