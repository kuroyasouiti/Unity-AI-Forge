...
| **Act** | 問題を修正してサイクルを閉じる | `component_crud(remove)` で不要コンポーネント除去, `event_wiring(wire)`, `prefab_crud(applyOverrides)` |
...
# Missing Script が検出された場合は component_crud / gameobject_crud で手動対処
```
...
```python
# Missing Script の対処: validate_integrity(all) で検出後、
# component_crud(remove) や gameobject_crud(delete) で手動修正
...
# 検出されたら component_crud(remove) や gameobject_crud(delete) で手動修正
...
# Missing Script の対処: component_crud(remove) や gameobject_crud(delete) で手動修正
...
正しい操作名は `missingScripts`, `nullReferences`, `brokenEvents`, `brokenPrefabs`, `all` です。checkMissingScripts や checkNullReferences 等は存在しません。
...
| ツール | テスト・検証での用途 |
|--------|-------------------|
| `unity_validate_integrity` | missingScripts / nullReferences / brokenEvents / brokenPrefabs / all |
...
| `unity_event_wiring` | wire / listEvents で再接続 |
