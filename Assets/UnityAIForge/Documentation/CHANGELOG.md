...
- **ConsoleLogHandler のコンパイルログ検出を改善**
  - ログタイプによるフィルタリング時に全エントリをスキャンするよう変更（以前は最近のエントリのみ）
  - コンパイルエラー/警告のメッセージ内容に基づくフォールバック検出を追加
  - `error CS`, `warning CS` パターンでコンパイルログを正確に分類

- **McpCliRegistry のトークン上書き防止**
  - `RegisterProjectViaJson` で既存の `MCP_BRIDGE_TOKEN` を保持するよう変更
  - `GetExistingToken` メソッドを追加して既存トークンを取得
  - User/Local/Project スコープに対応したトークン取得ロジック
