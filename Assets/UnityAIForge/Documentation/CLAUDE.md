...
Unity-AI-Forgeは、Model Context Protocol (MCP)を通じてAIアシスタントがUnity Editorをリアルタイムで操作するための開発ツールキットです。41のMCPツールを提供し、3層アーキテクチャ（High-Level GameKit / Mid-Level / Low-Level CRUD）で構成されています。
...
```
High-Level GameKit (9ツール)   ← 最優先: ゲームシステム一式を1コマンドで構築
  GameKit UI (1) / Logic Pillar (7) / GameKit Data (1)
Mid-Level          (18ツール)  ← バッチ操作・プリセット・開発サイクル（UI Toolkit含む）
Low-Level CRUD      (8ツール)  ← 個別のUnity操作
Utility             (5ツール)  ← 接続確認・コンパイル待機・イベント接続・プレイモード・コンソール
...
### High-Level GameKit（9ツール）


GameKitはUI・Logic・Systemsで構成されています。ハンドラーはコード生成でスタンドアロンC#スクリプトを生成し、Unity-AI-Forgeパッケージへのランタイム依存はありません。

**GameKit UI（1ツール）:**

| ツール名 | 用途 |
|----------|------|
| `unity_gamekit_ui` | UIコンポーネント (widgetType: command/binding/list/slot/selection) |

**GameKit Data（1ツール）:**

| ツール名 | 用途 |
|----------|------|
| `unity_gamekit_data` | プーリング、イベントチャンネル、データコンテナ、ランタイムセット (dataType: pool/eventChannel/dataContainer/runtimeSet) |
...
| `unity_camera_bundle` | カメラCRUD+プリセット |
| `unity_ui_foundation` | UI要素作成・表示/非表示・ツリー検査 |
...
UGUIベースの画面構築にはMid-Levelツールを使用します。
