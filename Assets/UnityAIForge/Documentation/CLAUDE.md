...
Unity-AI-Forgeは、Model Context Protocol (MCP)を通じてAIアシスタントがUnity Editorをリアルタイムで操作するための開発ツールキットです。47のMCPツールを提供し、3層アーキテクチャ（High-Level GameKit / Mid-Level / Low-Level CRUD）で構成されています。
...
```
High-Level GameKit (14ツール)  ← 最優先: ゲームシステム一式を1コマンドで構築
  UI Pillar (5) / Logic Pillar (7) / Systems (2)
