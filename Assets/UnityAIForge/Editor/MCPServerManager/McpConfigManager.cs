...
namespace MCP.Editor.ServerManager
{
    ...
    public static class McpConfigManager
    {
        ...
        private static string FindCursorConfigPath(string appData)
        {
            ...
            // 可能性1: ユーザーフォルダ直下の.cursor/mcp.json（新しい標準）
            var path1 = Path.Combine(userProfile, ".cursor", "mcp.json");
            if (File.Exists(path1)) return path1;
            
            // 可能性2: Cursor独自のMCP設定
            var path2 = Path.Combine(appData, "Cursor", "User", "globalStorage", "cursor", "mcp.json");
            if (File.Exists(path2)) return path2;
            
            // 可能性3: CursorのグローバルMCP設定
            var path3 = Path.Combine(appData, "Cursor", "User", "globalStorage", "cursor-mcp", "settings.json");
            if (File.Exists(path3)) return path3;
            
            // 可能性4: Cline統合（Roo Cline）
            ...
            // 可能性5: Cline統合（saoudrizwan）
            var path5 = Path.Combine(appData, "Cursor", "User", "globalStorage", 
                "saoudrizwan.claude-dev", "settings", "cline_mcp_settings.json");
            if (File.Exists(path5)) return path5;
            
            // 可能性6: Cursorのsettings.json（メイン設定ファイル）
            var path6 = Path.Combine(appData, "Cursor", "User", "settings.json");
            if (File.Exists(path6))
            {
                // settings.jsonがあれば、それを使用（mcpServersセクションがあるかチェック）
                try
                {
                    var content = File.ReadAllText(path6);
            ...
            // デフォルト: ユーザーフォルダ直下の.cursor/mcp.jsonを返す（最新の標準）
            return path1;
        }
