...
- 特殊ツール: 
  ...
  - `scriptManage`: 指定スクリプトのread（アウトライン+ソース全文）を生成してLLM対話に提供。解析時にRoslyn等を利用し、構文エラーが検出された場合はエラー位置とメッセージも返すことで編集前の安全確認を支援。 create操作ではMonoBehaviour/ScriptableObjectなどのテンプレート生成も対応し、名前空間やメソッド・フィールドの一括指定を可能にする。 update/delete操作では既存スクリプトへの部分的なテキスト修正や削除をdryRun付きで安全に実行できる。
